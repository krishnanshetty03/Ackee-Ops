...
import { ArrowLeft, Camera, Globe, Leaf, Package, Phone } from '../../components/icons'
...
export function FarmerApp({ farmerId, theme, onBack }: { farmerId: string; theme: Theme; onBack?: () => void }) {
  ...
  const startBonusRequest = useTallawahStore((st) => st.startBonusRequest)
  const attachProducePhoto = useTallawahStore((st) => st.attachProducePhoto)
  const skipProducePhoto = useTallawahStore((st) => st.skipProducePhoto)
  ...
  function handleQuickReply(kind: string, value: string) {
    ...
    if (value === 'attach_produce_photo') return attachProducePhoto(farmerId)
    if (value === 'skip_produce_photo') return skipProducePhoto(farmerId)
    if (value.startsWith('claim_bonus')) {
      const parts = value.split('_')
      const pct = parts[2] ?? '10'
      return startBonusRequest(farmerId, `${pct}% Bonus`)
    }
    if (value === 'my_requests') return showMyRequests(farmerId)
    ...
    <div className={s.screen}>
      ...
      <div className={s.composer}>
        <div className={s.quickMenu}>
          ...
          <button className={s.quickMenuBtn} onClick={() => attachProducePhoto(farmerId)}>
            <Camera size={13} /> Produce Photo
          </button>
