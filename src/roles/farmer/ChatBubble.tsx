  ...
  return (
    <div className={[s.row, outgoing ? s.rowOut : s.rowIn].join(' ')}>
      <div className={s.bubbleWrap}>
        ...
        ) : message.kind === 'photo_share' && message.photoUrl ? (
          <div className={s.locationBubble} style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.1)' }}>
            <img src={message.photoUrl} alt="Produce Verification Photo" style={{ width: '100%', height: 160, objectFit: 'cover', display: 'block' }} />
            <div className={s.locationCaption} style={{ background: '#fff', color: '#1a1a1a', fontWeight: 600 }}>
              <span style={{ color: '#25d366' }}>✔</span> Produce Verification Photo Attached
            </div>
          </div>
          ...
          <>
            ...
            <div className={s.card}>
              <div className={s.cardHead}>
                ...
                {message.requestSummary.bonusApplied && (
                  <span style={{ fontSize: 10, fontWeight: 700, background: '#25d366', color: '#fff', padding: '2px 6px', borderRadius: 4 }}>
                    🎁 {message.requestSummary.bonusApplied}
                  </span>
                )}
              </div>
              {message.requestSummary.producePhotoUrl && (
                <div style={{ padding: '6px 10px 0' }}>
                  <img
                    src={message.requestSummary.producePhotoUrl}
                    alt="Verified Produce"
