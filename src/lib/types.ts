...
export type ChatStage =
  ...
  | { step: 'awaiting_bags'; bonusTag?: string }
  | { step: 'awaiting_photo'; bags: number; bonusTag?: string }
  | { step: 'awaiting_branch'; bags: number; bonusTag?: string; producePhotoUrl?: string }
  | { step: 'awaiting_type'; bags: number; branchId: string; bonusTag?: string; producePhotoUrl?: string }
  | { step: 'awaiting_location'; bags: number; type: RequestType; branchId: string; bonusTag?: string; producePhotoUrl?: string }
  | { step: 'awaiting_dropoff_timing'; bags: number; type: RequestType; branchId: string; bonusTag?: string; producePhotoUrl?: string }
...
export interface FarmerInvite {
  id: string
  /** display name captured at invite time */
  name: string
  phone: string
  community: string
  /** epoch ms when staff sent the invite */
  invitedAt: number
  status: InviteStatus
  /** short hex token that appears in the simulated SMS link */
  token: string
  /** set once the farmer completes onboarding */
  acceptedFarmerId?: string
}
...
export const FARMER_TAG_OPTIONS = ['Preferred Supplier', 'High Volume', 'Reliable', 'Needs Follow-up', 'ND (Needs Delivery)', 'At Risk'] as const
