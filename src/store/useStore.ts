...
type Actions = {
  ...
  startBonusRequest: (farmerId: string, bonusTag?: string) => void
  sendBulkWhatsAppBroadcast: (targetFilter: string, messageText?: string, bonusPct?: number) => { count: number }
  attachProducePhoto: (farmerId: string, photoUrl?: string) => void
  skipProducePhoto: (farmerId: string) => void
  ...
  // ---- farmer invites ----
  sendInvite: (name: string, phone: string, community: string) => void
  acceptInvite: (token: string) => string | null // returns new farmerId
...
let _inviteCounter = 100
function nextInviteId() {
  return `INV-${++_inviteCounter}`
}

const SEED_INVITES: FarmerInvite[] = [
  {
    id: 'INV-001',
    name: 'Kweku Boateng',
    phone: '+233 24 441 8823',
    community: 'Ejisu',
    invitedAt: Date.now() - 3 * 3600_000,
    status: 'pending',
    token: 'a1b2c3d4',
  },
  {
  ...
  },
  {
    id: 'INV-003',
    name: 'Kofi Baffour',
    phone: '+233 20 223 7791',
    community: 'Bekwai',
    invitedAt: Date.now() - 72 * 3600_000,
    status: 'expired',
    token: 'c9d0e1f2',
  },
]
...
export const useTallawahStore = create<Store>((set, get) => ({
  ...
  chooseBags: (farmerId, bags) =>
    set((s) => {
      const farmer = s.farmers.find((f) => f.id === farmerId)
      const stage = s.chatStage[farmerId]
      const bonusTag = stage && 'bonusTag' in stage ? stage.bonusTag : undefined
      const farmerMsg = msg(farmerId, { from: 'farmer', kind: 'text', text: `${bags} bag${bags === 1 ? '' : 's'}` })
      const botMsg = msg(farmerId, {
        from: 'bot',
        kind: 'quick_replies',
        text: `📸 *PRODUCE VERIFICATION CHECK*\n\nGot it, ${bags} bag${bags === 1 ? '' : 's'} noted!\n\nPlease attach or snap a photo of your harvested ackee pods to verify freshness before routing.`,
        options: [
          { label: '📷 Attach Produce Photo', value: 'attach_produce_photo' },
          { label: '⏩ Skip Photo', value: 'skip_produce_photo' },
        ],
      })
      return {
        chat: pushChat(s.chat, farmerId, farmerMsg, botMsg),
        chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_photo', bags, bonusTag } },
      }
    }),

  attachProducePhoto: (farmerId, customPhotoUrl) =>
    set((s) => {
      const stage = s.chatStage[farmerId]
      const farmer = s.farmers.find((f) => f.id === farmerId)
      const lang = farmer?.language
      const t = copyFor(lang)
      const photoUrl = customPhotoUrl || `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="260" viewBox="0 0 400 260"><rect width="400" height="260" fill="%232c3e50"/><rect x="10" y="10" width="380" height="240" rx="8" fill="%2334495e"/><circle cx="140" cy="130" r="55" fill="%23e74c3c"/><circle cx="260" cy="120" r="50" fill="%23c0392b"/><path d="M125,110 Q140,80 155,110 Q140,140 125,110 Z" fill="%23f1c40f"/><path d="M245,100 Q260,70 275,100 Q260,130 245,100 Z" fill="%23f1c40f"/><circle cx="140" cy="100" r="14" fill="%23111"/><circle cx="260" cy="90" r="13" fill="%23111"/><rect x="25" y="25" width="130" height="26" rx="13" fill="rgba(37,211,102,0.95)"/><text x="90" y="42" font-family="sans-serif" font-size="12" font-weight="bold" fill="%23fff" text-anchor="middle">✔ VERIFIED FRESH</text><text x="370" y="235" font-family="sans-serif" font-size="11" fill="rgba(255,255,255,0.7)" text-anchor="end">Tallawah Produce QA • Ashanti Region</text></svg>`
      const bags = stage && 'bags' in stage ? stage.bags : 10
      const bonusTag = stage && 'bonusTag' in stage ? stage.bonusTag : undefined

      const photoMsg = msg(farmerId, {
        from: 'farmer',
        kind: 'photo_share',
        photoUrl,
        text: '📷 Produce photo attached (fresh harvest)',
      })

      const botAck = msg(farmerId, {
        from: 'bot',
        kind: 'text',
        text: '✅ *Produce Verification Passed!* Freshness grade: A (Optimal arils).',
      })

      const botBranch = msg(farmerId, {
        from: 'bot',
        kind: 'quick_replies',
      ...
      return {
        chat: pushChat(s.chat, farmerId, photoMsg, botAck, botBranch),
        chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_branch', bags, bonusTag, producePhotoUrl: photoUrl } },
      }
    }),

  skipProducePhoto: (farmerId) =>
    set((s) => {
      const stage = s.chatStage[farmerId]
      if (!stage || stage.step !== 'awaiting_photo') return {}
      const farmer = s.farmers.find((f) => f.id === farmerId)
      const t = copyFor(farmer?.language)
      const farmerMsg = msg(farmerId, { from: 'farmer', kind: 'text', text: '⏩ Skip photo' })
      const botMsg = msg(farmerId, {
        from: 'bot',
        kind: 'quick_replies',
        text: t.bagsNoted(stage.bags),
        options: branchOptions(s.branches),
      })
      return {
        chat: pushChat(s.chat, farmerId, farmerMsg, botMsg),
        chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_branch', bags: stage.bags, bonusTag: stage.bonusTag } },
      }
    }),
  ...
  chooseBranch: (farmerId, branchId) =>
    set((s) => {
      ...
      return {
        chat: pushChat(s.chat, farmerId, farmerMsg, botMsg),
        chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_type', bags: stage.bags, branchId, bonusTag: stage.bonusTag, producePhotoUrl: stage.producePhotoUrl } },
      }
    }),
    ...
    set((s) => {
      ...
      if (type === 'staff_pickup') {
        ...
        return {
          ...
          chatStage: {
            ...s.chatStage,
            [farmerId]: { step: 'awaiting_location', bags: stage.bags, type, branchId: stage.branchId, bonusTag: stage.bonusTag, producePhotoUrl: stage.producePhotoUrl },
          },
        }
      }
      ...
      return {
        ...
        chatStage: {
          ...s.chatStage,
          [farmerId]: { step: 'awaiting_dropoff_timing', bags: stage.bags, type, branchId: stage.branchId, bonusTag: stage.bonusTag, producePhotoUrl: stage.producePhotoUrl },
        },
      }
    }),
  ...
  startBonusRequest: (farmerId, bonusTag = '10% Bonus') =>
    set((s) => {
      const farmer = s.farmers.find((f) => f.id === farmerId)!
      const already = s.chat[farmerId]?.length ?? 0
      if (already === 0 && !farmer.language) {
        ...
        return {
          chat: pushChat(s.chat, farmerId, ask),
          chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_language', bonusTag } },
        }
      }
      const t = copyFor(farmer.language)
      const bonusHeader = `🎁 *${bonusTag.toUpperCase()} ACTIVATED!*\n\n`
      const ask = msg(farmerId, { from: 'bot', kind: 'quick_replies', text: bonusHeader + t.askBags(farmer.name), options: bagQuickReplyOptions() })
      return {
        chat: pushChat(s.chat, farmerId, ask),
        chatStage: { ...s.chatStage, [farmerId]: { step: 'awaiting_bags', bonusTag } },
      }
    }),

  sendBulkWhatsAppBroadcast: (targetFilter, messageText, bonusPct = 10) => {
    const s = get()
    let targeted = s.farmers

    if (targetFilter === 'nd') {
      targeted = s.farmers.filter(
        (f) =>
          f.tags.some((t) => t.toLowerCase().includes('nd') || t.toLowerCase().includes('delivery')) ||
          s.farmerNotes.some((n) => n.farmerId === f.id && n.text.toLowerCase().includes('nd')),
      )
      if (targeted.length === 0) {
        targeted = s.farmers.filter((f) => f.tags.includes('ND (Needs Delivery)') || f.community === 'Ejisu' || f.community === 'Bekwai')
      }
    } else if (targetFilter === 'preferred') {
      targeted = s.farmers.filter((f) => f.tags.includes('Preferred Supplier'))
    } else if (targetFilter === 'high_volume') {
      targeted = s.farmers.filter((f) => f.tags.includes('High Volume'))
    } else if (targetFilter !== 'all') {
      targeted = s.farmers.filter((f) => f.community.toLowerCase() === targetFilter.toLowerCase())
    }

    if (targeted.length === 0) targeted = s.farmers

    const defaultText = `🎁 *SPECIAL ${bonusPct}% HARVEST BONUS THIS WEEK!*\n\nTallawah Foods Ghana is paying an extra ${bonusPct}% cash bonus on all ackee orders placed this week.\n\nTap below to place your order directly on WhatsApp and lock in your ${bonusPct}% payout boost!`

    const finalMsg = messageText?.trim() ? messageText.trim() : defaultText

    let newChat = { ...s.chat }
    for (const farmer of targeted) {
      const bMsg = msg(farmer.id, {
        from: 'bot',
        kind: 'quick_replies',
        text: finalMsg,
        options: [
          { label: `🎁 Order Now (${bonusPct}% Bonus)`, value: `claim_bonus_${bonusPct}` },
          { label: '📦 My Requests', value: 'my_requests' },
        ],
      })
      newChat = pushChat(newChat, farmer.id, bMsg)
    }
    ...
    const notif = notify({
      ...
      body: `${bonusPct}% Bonus campaign sent to ${targeted.length} farmer${targeted.length === 1 ? '' : 's'}.`,
    })
  ...
  _finalizeRequest: (farmerId, bags, branchId, type, location, dropoffTiming) => {
    ...
    const farmer = s.farmers.find((f) => f.id === farmerId)!
    const stage = s.chatStage[farmerId]
    const bonusApplied = stage && 'bonusTag' in stage && stage.bonusTag ? stage.bonusTag : undefined
    const producePhotoUrl = stage && 'producePhotoUrl' in stage && stage.producePhotoUrl ? stage.producePhotoUrl : undefined
    const t = copyFor(farmer.language)
    const branch = s.branches.find((b) => b.id === branchId)
    ...
    const cardMsg = msg(farmerId, {
      from: 'bot',
      kind: 'request_card',
      text: t.requestConfirmed(id) + (bonusApplied ? `\n\n🎁 *${bonusApplied} Applied!*` : '') + (producePhotoUrl ? '\n📷 *Produce Photo Verified*' : ''),
      requestSummary: { requestId: id, bags, type, branchName: branch?.name, status: 'unassigned', dropoffTiming, bonusApplied, producePhotoUrl },
    })
    const n = notify({
      audience: 'staff',
      kind: 'request',
      title: bonusApplied ? `New order (${bonusApplied})` : 'New pickup request',
      body: `${farmer.name} — ${bags} bags (${type === 'staff_pickup' ? 'team pickup' : 'self-drop'})${producePhotoUrl ? ' · 📷 Photo Verified' : ''}`,
    })
    set((st) => ({
  ...
  // ================= FARMER INVITES =================
  sendInvite: (name, phone, community) => {
    ...
    const n = notify({ audience: 'staff', kind: 'request', title: 'Invite sent', body: `${trimmedName} — ${trimmedPhone}` })
    set((st) => ({ invites: [invite, ...st.invites], notifications: [n, ...st.notifications] }))
  },

  acceptInvite: (token) => {
    const st0 = get()
    const invite = st0.invites.find((i) => i.token === token && i.status === 'pending')
    ...
    const initials = invite.name
      ...
      .join('')
      .toUpperCase()
    const farmerId = `FM-${String(Date.now()).slice(-6)}`
    ...
    const n = notify({ audience: 'staff', kind: 'request', title: 'New farmer joined', body: `${invite.name} accepted their invite and joined the platform.` })
    set((st) => ({
      invites: st.invites.map((i) => (i.token === token ? { ...i, status: 'accepted', acceptedFarmerId: farmerId } : i)),
      farmers: [...st.farmers, newFarmer],
      notifications: [n, ...st.notifications],
    }))
    return farmerId
  },
  ...
  resendInvite: (inviteId) =>
    set((st) => ({
      invites: st.invites.map((i) => (i.id === inviteId ? { ...i, invitedAt: Date.now(), status: 'pending' } : i)),
    })),

  revokeInvite: (inviteId) =>
    set((st) => ({
      invites: st.invites.map((i) => (i.id === inviteId ? { ...i, status: 'expired' } : i)),
    })),
...
export function startSync() {
  ...
  useTallawahStore.subscribe((state) => {
    ...
    const { version, farmers, drivers, vehicles, branches, staff, requests, routes, shipments, stock, exceptions, notifications, chat, chatStage, farmerNotes, followUpTasks, invites, now } = state
    persistAndBroadcast({ version, farmers, drivers, vehicles, branches, staff, requests, routes, shipments, stock, exceptions, notifications, chat, chatStage, farmerNotes, followUpTasks, invites, now })
  })
